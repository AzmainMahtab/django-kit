...
class OwnerListCreateView(APIView):
    ...
    def post(self, request):
        ...
        try:
            owner = get_owner().create_owner(**serializer.validated_data)
...
class OwnerDetailView(APIView):
    ...
    def get(self, request, owner_uuid: str):
        try:
            owner = get_owner().get_owner_by_uuid(uuid=owner_uuid)
...
class OwnerByUserView(APIView):
    ...
    def get(self, request, user_id: int):
        try:
            owner = get_owner().get_owner_by_user_id(user_id=user_id)
        except OwnerNotFoundError as exc:
            return Response(
                {"detail": str(exc), "code": exc.default_code},
                status=status.HTTP_404_NOT_FOUND,
