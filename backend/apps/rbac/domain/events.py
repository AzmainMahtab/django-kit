...
class PermissionCreated(DomainEvent):
    event_type: str = "rbac.permission_created"


@dataclass
class RoleCreated(DomainEvent):
    event_type: str = "rbac.role_created"


@dataclass
class RoleAssigned(DomainEvent):
    event_type: str = "rbac.role_assigned"


@dataclass
class RoleRevoked(DomainEvent):
    event_type: str = "rbac.role_revoked"


@dataclass
class PermissionAssignedToRole(DomainEvent):
    event_type: str = "rbac.permission_assigned_to_role"


@dataclass
class PermissionRevokedFromRole(DomainEvent):
    event_type: str = "rbac.permission_revoked_from_role"
